  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex items-center justify-center p-4">
        <div className="max-w-2xl w-full">
            <Link to="/" className="flex justify-center items-center mb-6 text-gray-700 dark:text-gray-200 hover:text-primary">
                <ICONS.logo className="h-8 w-8 text-primary" />
                <span className="ml-3 text-2xl font-bold">VitalLink</span>
            </Link>
            <Card className="w-full">
                <div className="p-6">
                <h2 className="text-2xl font-bold text-center mb-6 text-gray-800 dark:text-white">Certificate Verification</h2>
                {record && hospital ? (
                    ...
                    <div className="border-t dark:border-gray-700 pt-4">
                        <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-4">
                            <div className="sm:col-span-2">
                                ...
                                <dd className="mt-1 text-lg font-semibold text-gray-900 dark:text-white">{record.fullName}</dd>
                            </div>
                            <div>
                                <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Certificate ID</dt>
                                <dd className="mt-1 text-gray-900 dark:text-white">{record.certificateId}</dd>
                            </div>
                            <div>
                                <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Record Type</dt>
                                <dd className="mt-1 text-gray-900 dark:text-white capitalize">{record.recordType.toLowerCase()}</dd>
                            </div>
                            <div>
                                <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Date</dt>
                                <dd className="mt-1 text-gray-900 dark:text-white">{record.date}</dd>
                            </div>
                             <div>
                                <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Registered By</dt>
                                <dd className="mt-1 text-gray-900 dark:text-white">{hospital.name}</dd>
                            </div>
                        </dl>
                    </div>
                    </div>
